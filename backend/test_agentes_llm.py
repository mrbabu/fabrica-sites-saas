#!/usr/bin/env python3
"""
Benchmark de qualidade do Agente Construtor contra LLM REAL.

Roda 50+ nichos de verdade e reporta taxa de sucesso — é a medição que
responde "o modelo está produzindo config válido?". Custa dinheiro/tempo e
depende de rede, então NÃO entra no gate de commit: fica na suíte lenta
(`python backend/qa.py --lentas`).

Era o conteúdo de test_agentes.py até a Fase 3 do QA. Foi movido pra cá,
sem perda de capacidade, quando test_agentes.py virou suíte determinística
de regressão (dublês, zero rede). São perguntas diferentes:
  - test_agentes.py      -> o pipeline trata as respostas corretamente?
  - test_agentes_llm.py  -> o modelo real produz respostas boas?

Uso: python backend/test_agentes_llm.py [limite_nichos]
"""
...
from ai_provider import ErroProvedorIA, obter_ai_provider
...
def main():
    ...
    # Confere que EXISTE uma cadeia de provedores utilizável, em vez de exigir
    # um provedor específico. A checagem anterior era `if not
    # os.getenv("ANTHROPIC_API_KEY"): sys.exit(1)`, o que abortava o benchmark
    # mesmo com a aplicação perfeitamente funcional via Gemini, NVIDIA NIM ou
    # Ollama local — a cadeia real é gemini -> nvidia_nim -> anthropic ->
    # ollama (ai_provider.py:_ORDEM_PADRAO), e Anthropic é só um dos elos.
    # Disponibilidade de fato só se prova chamando: quem decide é o fallback.
    try:
        provedor = obter_ai_provider()
    except ErroProvedorIA as e:
        print(f"❌ Nenhum provedor de IA configurado: {e}")
        sys.exit(1)

    print(f"🔗 Cadeia de provedores: {' -> '.join(provedor.ordem)}")
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print(f"Uso: python test_agentes_llm.py [limite_nichos]")
            sys.exit(1)
