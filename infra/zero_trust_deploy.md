# Deploy Zero Trust — Instância Ampere (152.67.59.214)
...
**Estado atual (sessão de 2026-07-11/12):** instância recriada do zero —
`VM.Standard.A2.Flex` (Ampere, ARM64, 4 OCPU/24GB), Ubuntu 20.04, IP
público `152.67.59.214`, região `sa-saopaulo-1`. A VM anterior
(`fabrica-postgres`, 147.15.73.239) foi terminada; essa é uma máquina nova
e vazia — Postgres é criado do zero pelo próprio `docker-compose.prod.yml`
(serviço `db`), não reaproveita nada.
...
## 1. Instalar Docker e Tailscale na VM

Via SSH público (ainda liberado nesse momento):

```bash
# Docker
curl -fsSL https://get.docker.com | sudo sh
sudo usermod -aG docker $USER
# desconecta e reconecta o SSH depois deste comando, pra valer o grupo novo
...
Groups** anexados ao VNIC (nessa instância: a Default Security List da
`vcn-20260710-1156` e a `ig-quick-action-NSG` criada pela ação rápida de
conectividade).
...
```bash
nmap -Pn -p 22,80,443 152.67.59.214
...
"zero exposição pública". UFW no host é uma segunda camada:
...
```bash
git clone https://github.com/mrbabu/fabrica-sites-saas.git
cd fabrica-sites-saas
cp .env.example .env
nano .env   # preencher com valores reais, ver seção abaixo
docker compose -f docker-compose.prod.yml up -d --build
docker compose -f docker-compose.prod.yml logs -f backend
```

Preencher no `.env` antes de subir: `POSTGRES_PASSWORD` (senha forte
real, não a do exemplo — `DATABASE_URL` é montada automaticamente pelo
compose a partir dela), `CLOUDFLARE_TUNNEL_TOKEN` (obtido no passo 5),
mais as chaves de IA que já existem hoje (`GEMINI_API_KEY` etc.),
`WEBHOOK_API_KEY` e `WHATSAPP_VERIFY_TOKEN`.
